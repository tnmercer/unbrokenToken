...
#Home page logo
# image = Image.open('ubtlogo.png')
# st.image(image)
#video_bytes = video_file.read()
#st.video(video_bytes)
#st.markdown("---")

file_ = open("ubtoken_clean.gif", "rb")
contents = file_.read()
data_url = base64.b64encode(contents).decode("utf-8")
file_.close()

st.markdown(
    f'<img src="data:image/gif;base64,{data_url}" alt="UBT gif">',
...
if st.button("Random Winner"):
    account_number = random.choice(accounts[1:])
    st.write(f"{account_number}")
    st.snow()
